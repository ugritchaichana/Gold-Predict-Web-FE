...
const MonthlyPredictions = ({ monthlyPredictions, monthlyChartTab, setMonthlyChartTab }) => {
  ...
  const rowsPerPage = 6;
  const totalPages = Math.ceil(monthlyPredictions.length / rowsPerPage);

  // Update paginated data when page changes or predictions change
  useEffect(() => {
    const startIndex = (currentPage - 1) * rowsPerPage;
    const endIndex = startIndex + rowsPerPage;
    setPaginatedData(monthlyPredictions.slice(startIndex, endIndex));
  }, [currentPage, monthlyPredictions]);
